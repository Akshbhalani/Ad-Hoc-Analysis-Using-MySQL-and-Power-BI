...
WITH merged_table AS 
	(SELECT dim_product.*,fact_sales_monthly.fiscal_year FROM dim_product 
    JOIN fact_sales_monthly ON dim_product.product_code = fact_sales_monthly.product_code), 

unique_count_table AS 
	(SELECT segment,fiscal_year,COUNT(DISTINCT product_code) as product_count FROM merged_table 
	GROUP BY segment, fiscal_year), fiscal_year_count_table AS 
	...
	FROM unique_count_table 
	GROUP BY segment) 

SELECT segment,product_count_2020,product_count_2021,product_count_2021 - product_count_2020 as difference 
FROM fiscal_year_count_table;


WITH merged_table AS 
	(SELECT dim_product.product_code, dim_product.product, dim_product.variant, fact_manufacturing_cost.manufacturing_cost
	FROM dim_product 
	JOIN fact_manufacturing_cost ON dim_product.product_code = fact_manufacturing_cost.product_code)
SELECT * FROM merged_table 
WHERE manufacturing_cost = (SELECT MAX(manufacturing_cost) FROM merged_table) 
	OR manufacturing_cost = (SELECT MIN(manufacturing_cost) FROM merged_table) 
    ORDER BY manufacturing_cost DESC, product_code;
...
YEAR(fact_sales_monthly.date) year, 
SUM(fact_gross_price.gross_price * fact_sales_monthly.sold_quantity) gross_sales_amount 
FROM fact_sales_monthly 
...
SELECT CONCAT(QUARTER(date), 'Q', YEAR(date)) AS Quarter, SUM(sold_quantity) AS Total_Sold_Quantity 
FROM fact_sales_monthly 
...
WITH cte AS 
	(SELECT dim_customer.channel channel,SUM(fact_sales_monthly.sold_quantity * fact_gross_price.gross_price) AS gross_sales_mln
	FROM fact_sales_monthly 
...
SELECT channel, gross_sales_mln, ROUND(gross_sales_mln*100/(SELECT SUM(gross_sales_mln) FROM cte),2)percentage 
FROM cte
...
WITH merged_data AS 
	(SELECT dim_product.division, dim_product.product_code, dim_product.product, fact_sales_monthly.sold_quantity, fact_sales_monthly.fiscal_year
	FROM dim_product 
	INNER JOIN fact_sales_monthly ON dim_product.product_code = fact_sales_monthly.product_code),
    
aggregated_data AS 
	(SELECT division, MAX(Total_Sold_Quantity) AS Max_Total_Sold_Quantity 
    FROM (SELECT product_code, product, division, fiscal_year, SUM(sold_quantity) AS Total_Sold_Quantity 
	FROM merged_data 
    WHERE fiscal_year = 2021 
    GROUP BY product_code, product, division, fiscal_year) 
    subquery GROUP BY division), 

ranked_data AS 
	(SELECT division, product_code, product, fiscal_year, Total_Sold_Quantity, 
    ROW_NUMBER() 
    OVER (ORDER BY Total_Sold_Quantity DESC) AS rank_order
	FROM (SELECT division, product_code, product, fiscal_year, Total_Sold_Quantity 
    FROM (SELECT product_code, product, division, fiscal_year, SUM(sold_quantity) AS Total_Sold_Quantity 
    FROM merged_data 
    WHERE fiscal_year = 2021 
    GROUP BY product_code, product, division, fiscal_year) subquery
